...
minetest.register_node("default:basic_flame", {
	...
	on_construct = function(pos)
		...
		minetest.after(0, default.update_sounds_around, pos)
	end,
...
minetest.register_node("default:permanent_flame", {
...
minetest.register_node("default:underground_fire", {
	description = "Underground Permanent Flame",
	drawtype = "firelike",
	tiles = {
		{
			name = "fire_basic_flame_animated.png",
...
minetest.override_item("default:rack", {
	after_destruct = function(pos, oldnode)
		pos.y = pos.y + 1
		if minetest.get_node(pos).name == "fire:permanent_flame" then
			minetest.remove_node(pos)
...
default.D = 6 -- size of sound areas

function default.get_area_p0p1(pos)
	local p0 = {
		x = math.floor(pos.x / default.D) * default.D,
		y = math.floor(pos.y / default.D) * default.D,
		z = math.floor(pos.z / default.D) * default.D,
	}
	local p1 = {
		x = p0.x +default.D - 1,
		y = p0.y + default.D - 1,
...
function default.update_sounds_around(pos)
	local p0, p1 = default.get_area_p0p1(pos)
	...
	local sound = default.sounds[p0_hash]
	if not sound then
		if should_have_sound then
			default.sounds[p0_hash] = {
...
minetest.register_abm({
	nodenames = {"default:basic_flame", "default:permanent_flame"},
...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if p then
			...
			if default.flame_should_extinguish(p) then
				return
			end
			local p2 = default.find_pos_for_flame_around(p)
			if p2 then
